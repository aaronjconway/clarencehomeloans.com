<script lang="ts">
	...
	//hashmap to help with duplicate file uploads
	// adds in (count) to end of file if dupe

	let c: Record<string, number> = {};
	//func for adding file number if dupes
	const t = (x: string, n: number) => x + ' (' + n + ')';
	//load the hash map of num: file
	...
	uppy.on('file-added', (file) => {
		const supabaseMetadata = {
			...
			//why can't we get the user from the user instead of session?
